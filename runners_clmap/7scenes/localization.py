...
from plp_association import parse_config as parse_plp_config
from plp_association import run_plp_association
...
def parse_config():
    ...
    arg_parser.add_argument('-c', '--config_file', type=str,
                            default='cfgs_clmap/localization/7scenes.yaml', help='config file')
    arg_parser.add_argument('--default_config_file', type=str,
                            default='cfgs_clmap/localization/default.yaml', help='default config file')
...
def main():
    ...
    # Output path for LIMAP/CLMAP results (tmp)
    if cfg['output_dir'] is None:
        cfg['output_dir'] = 'tmp/7scenes/{}'.format(args.scene)
    # Output folder for LIMAP/CLMAP linetracks (in tmp)
    ...
    # Some paths useful for LIMAP/CLMAP localization too
    ...
    ##########################################################
    # [B] LIMAP/CLMAP triangulation/fitnmerge for database line tracks
    ##########################################################
    ...
    if not args.use_dense_depth:
        ...
        if not cfg['skip_exists'] or not os.path.exists(finaltracks_dir):
            ...
            print('Running CLMAP...')
            _, colmap_model_path = _runners_clmap.line_triangulation(
                cfg, imagecols_train, neighbors=neighbors, ranges=ranges)
            # joint optimization
            plp_input_dir = os.path.join(cfg["output_dir"], cfg["output_folder"])
            plp_cfg = parse_plp_config()
            plp_cfg["load_vpdet"] = True  # to save time
            plp_cfg["load_dir"] = cfg["output_dir"]
            plp_cfg["output_dir"] = cfg["output_dir"]
            plp_cfg["output_folder"] = "plp_finaltracks"
            _, _, linetracks_db, _, _, _, _, _, _ = run_plp_association(plp_cfg, plp_input_dir, colmap_model_path)
        else:
            linetracks_db = limapio.read_folder_linetracks(finaltracks_dir)
            logger.info(f'Loaded CLMAP triangulation result from {finaltracks_dir}')
    else:
        finaltracks_dir = os.path.join(cfg['output_dir'], cfg['output_folder'])
