...
def read_scene_7scenes(cfg, root_path, model_path, image_path, n_neighbors=20):
    ...
    if cfg['info_path'] is None:
        imagecols, neighbors, ranges = _psfm.read_infos_colmap(
            cfg['sfm'], root_path, model_path, image_path, n_neighbors=n_neighbors)
        with open(os.path.join(output_dir, metainfos_filename), 'wb') as f:
            np.savez(f, imagecols_np=imagecols.as_dict(),
                     neighbors=neighbors, ranges=ranges)
    else:
        with open(cfg['info_path'], 'rb') as f:
            data = np.load(f, allow_pickle=True)
            imagecols_np, neighbors, ranges = data['imagecols_np'].item(
            ), data['neighbors'].item(), data['ranges']
...
def get_result_filenames(cfg, use_dense_depth=False):
    ...
    if ransac_cfg['method'] != None:
        ...
        ransac_postfix += '_{}'.format(ransac_cfg['thres'] if ransac_cfg['method'] !=
                                       'hybrid' else '{}-{}'.format(ransac_cfg['thres_point'], ransac_cfg['thres_line']))
    ...
    results_joint = 'results_{}_joint_{}{}{}{}{}.txt'.format(
        ...
        '{}_'.format(cfg['reprojection_filter']
                     ) if cfg['reprojection_filter'] is not None else '',
...
def run_hloc_7scenes(cfg, dataset, scene, results_file, test_list, num_covis=30, use_dense_depth=False, logger=None):
    ...
    retrieval_path = dataset / \
        '7scenes_densevlad_retrieval_top_10' / f'{scene}_top10.txt'
    feature_conf = {
